...
class PageSexAge(BasePage):
    ...
    def fun_extras(cls):
        ...
        cls.top_row_cols[1].markdown("<div style='margin-top:60x;'></div>", unsafe_allow_html=True)
        cls.top_row_cols[1].markdown(""" <style> div[data-testid="stButton"] { padding-bottom: 5px; } </style> """, unsafe_allow_html=True)

        cls.top_row_cols[1].button("Quick selection for  total age dependency ratio",  key="quick_selection_total_age_group")
        cls.top_row_cols[1].button("Quick selection for child age dependency ratio",  key="quick_selection_child_age_group")
        cls.top_row_cols[1].button("Quick selection for old-age dependency ratio",  key="quick_selection_old_age_group")
    ...
    def quick_selection(cls,feature_name,nom_denom_key_suffix):
        ...
        if feature_name=="age" and (st.session_state["quick_selection_total_age_group"] or st.session_state["quick_selection_child_age_group"] or  st.session_state["quick_selection_old_age_group"]):
            if nom_denom_key_suffix=="denominator":
                cls.set_checkbox_values_for_quick_selection(working_age_groups, nom_denom_key_suffix,  feature_name)
            else:
                if st.session_state["quick_selection_total_age_group"]:
                    cls.set_checkbox_values_for_quick_selection(child_age_groups+elderly_age_groups , nom_denom_key_suffix, feature_name)
                elif st.session_state["quick_selection_child_age_group"]:
                    cls.set_checkbox_values_for_quick_selection(child_age_groups, nom_denom_key_suffix,  feature_name)
                elif st.session_state["quick_selection_old_age_group"]:
                    cls.set_checkbox_values_for_quick_selection(elderly_age_groups, nom_denom_key_suffix,  feature_name)

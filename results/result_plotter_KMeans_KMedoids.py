...
def load_results(engine_name):
    ...
    for scaler, regex in pattern_map.items():
        ...
        dfs[scaler] = df
    return dfs
def plot_k_analysis(engines, METRICS, pattern_map, scaler_labels,title_word,fill_std=False):
    n_rows = 1
    n_cols = len(engines)
...
def save_max_silhouette (engine):
    dfs = load_results(engine[0])
    df = pd.concat(dfs, axis=0, ignore_index=True)

    print("indeks:",df.columns)
    ...
    df_best[columns].round(2).T.to_csv(f"files/{engine[0]}.csv", index=True)
    return df_best
...
fig = plot_k_analysis(engines, METRICS, pattern_map, scaler_labels,"stability")
...
fig = plot_k_analysis(engines, METRICS, pattern_map, scaler_labels,"seperability")
fig.savefig(f"files/seperability-{engines[0][1]}-{engines[1][1]}.png", dpi=300, bbox_inches="tight")

plt.show()
for engine in engines:
    print(save_max_silhouette(engine).head())

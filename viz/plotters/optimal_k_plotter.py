...
class OptimalKPlotter:
    ...
    @staticmethod
    def plot_optimal_k_analysis(num_seeds_to_plot,k_values,random_states,metrics_all,metrics_mean,ari_mean,ari_std,consensus_indices):
        ...
        for i, random_state in enumerate(random_states[:num_seeds_to_plot]):
            # Inertia
            axs[i, 0].plot(k_values,  metrics_all["Inertia"][i], 'bo-')
            elbow = KneeLocator(k_values,  metrics_all["Inertia"][i], curve='convex', direction='decreasing')
            if elbow.elbow:
            ...
            # Silhouette Score
            axs[i, 1].plot(k_values,  metrics_all["Silhouette"][i], 'ro-')
            axs[i, 1].set_title(f'Seed {random_state}: {column_titles[1]}')

            # Davies-Bouldin Index
            axs[i, 2].plot(k_values, metrics_all["Davies-Bouldin"][i], 'co-')
            axs[i, 2].set_title(f'Seed {random_state}: {column_titles[2]}')


        # Plot mean metrics
        axs[num_seeds_to_plot, 0].plot(k_values, metrics_mean["Inertia"], 'bo-')
        ...
        axs[num_seeds_to_plot, 1].plot(k_values, metrics_mean["Silhouette"], 'ro-')
        ...
        axs[num_seeds_to_plot, 2].plot(k_values, metrics_mean["Davies-Bouldin"], 'co-')
        optimal_k_mean_db = k_values[np.argmin( metrics_mean["Davies-Bouldin"])]
        axs[num_seeds_to_plot, 2].axvline(x=optimal_k_mean_db, color='r', linestyle='--')
        ...
        axs[num_seeds_to_plot+1, 2].set_title(f'Mean: {column_titles[3]}')
